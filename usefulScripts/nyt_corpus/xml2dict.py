...
class getInfo(object):
	...
	def __init__(self, dir_path):
		...
		self.info = redis.StrictRedis(host ='localhost', port = 6379, db =4)
						...
						for day in os.listdir(self.path + year + '/' + month + '/'):
							if day != '.DS_Store':
								for news in os.listdir(self.path + year + '/' + month + '/' + day + '/'):
									if news != '.DS_Store':
										self.addXML2dict(self.path + year + '/' + month + '/' + day + '/' + news , news[:-4])
										count  = count + 1
									if count % 10000 ==0:
										logger.info("PROGRESS --> @ %d " , count)


	def addXML2dict(self, file_path, news_no):
		root = etree.parse(open(file_path, "r"))
		self.info.set(news_no, getDictionary(root))

		
def getDictionary(root):
...
if __name__ =='__main__':
	...
	info = getInfo(dir_path).enterInfo()

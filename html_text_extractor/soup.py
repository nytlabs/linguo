...
class Blobber(object):
	def __init__(self):
		...
		self.classifier =  pickle.load(open('classifier.pk'))
...
class Classifier(object):
	...
	def featureSet(self):
		input_data = [(classifier_features(observation), observation['good/bad']) for observation in self.data]
		a= numpy.random.randint(0, self.data_size, int(self.data_size*0.8))
...
def classifier_features( observation):
	features = {}
	features['.com/'] = 1 if observation['url'][-5:] in ['.com/'] else 0
	...
	features['density'] = observation['density']
	return features

def nn_features(observation):
	a = 1 if observation['url'][-5:] in ['.com/'] else 0
	b = 1 if observation['class'] else 0
	c = observation['no_tags']
	d = observation['pos']
	return (a,b,c,d)
...
'''
from pybrain.datasets import SupervisedDataSet as dr
ds = dr(4,1)
from pybrain.tools.shortcuts import buildNetwork as bn
net = bn(4,3,1)
for i in self.data:
	ds.addSample(nn_features(i),(i['good/bad'],))
...
prediction: net.activate([0,0,6,1])
'''
